    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:shotGoods:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="shotGoodsList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="跟拍商品编号" align="center" prop="shotId" />
...
<!--    商家对话框-->
    ...
    <!--    套餐对话框-->
    <el-dialog title="选择套餐" :visible.sync="packageDialog">
      <el-table :data="packageList">
        <el-table-column property="applicableProduct" label="商品编号" width="150"></el-table-column>
        <el-table-column property="packageName" label="套餐名称" width="150"></el-table-column>
        <el-table-column property="packagePrice" label="套餐价格" width="200"></el-table-column>
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button
              size="mini"
    ...
    <el-dialog title="请确认订单信息" :visible.sync="orderFormVisible">
      <el-form :model="orderParams">
        <el-form-item label="商品编号" :label-width="formLabelWidth">
          <el-input v-model="orderParams.shotId" autocomplete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="跟拍日期" :label-width="formLabelWidth">
          <el-date-picker
...
<script>
import { listShotGoods, getShotGoods, delShotGoods, addShotGoods, updateShotGoods,listBusiness,listPackage } from "@/api/ShotGoods/ShotGoods";
import {addShotOrder} from "../../api/ShotGoods/ShotGoods";
...
export default {
  ...
  methods: {
    ...
    orderSubmit(){
      ...
      addShotOrder(this.orderParams).then(response =>{
        this.$modal.msgSuccess("下单成功");
      })
    },
    orderGenerate(row){
      // this.orderRset()
      if ((this.orderParams.productName!=row.productName)||(this.orderParams.shotId!=row.shotId)){
    ...
    packageSelect(row){
      // this.packageRest()
      this.orderParams.price=row.packagePrice;
      this.orderParams.pack=row.packageName;
      this.orderParams.shotId=row.applicableProduct;
      this.packageDialog=false;
    },
    businessSelect(row){
      // this.businessRest()
      this.orderParams.productName=row.productName;
      this.orderParams.shopName=row.shopName;
      this.businessDialog=false;
    },
    business(row){
       this.businessQuery.applicableProduct= row.shotId
    ...
    package(row){
      this.packageQuery.applicableProduct = row.shotId
      listPackage(this.packageQuery).then(response =>{
    ...
    getList() {
      this.loading = true;
      listShotGoods(this.queryParams).then(response => {
        this.shotGoodsList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.shotId)
    ...
    handleUpdate(row) {
      this.reset();
      const shotId = row.shotId || this.ids
      getShotGoods(shotId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.shotId != null) {
            updateShotGoods(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addShotGoods(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const shotIds = row.shotId || this.ids;
      this.$modal.confirm('是否确认删除预约跟拍商品编号为"' + shotIds + '"的数据项？').then(function() {
        return delShotGoods(shotIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/shotGoods/export', {
        ...this.queryParams
      }, `shotGoods_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
